          ...
          {profiles.map((p) => (
            <div key={p.id} className="bg-white border border-zinc-200 p-4">
              <div className="flex items-start justify-between gap-2">
                <Link href={`/search?manager=${p.id}`} className="font-bold text-[#C0001E] hover:underline truncate">
                  {p.full_name}
                </Link>
              ...
              <div className="mt-3 pt-3 border-t border-zinc-100">
                <Link href={`/search?manager=${p.id}`} className="text-xs font-medium text-zinc-700 hover:text-[#C0001E] hover:underline">
                  Ver peleadores →
                </Link>
              </div>
            </div>
