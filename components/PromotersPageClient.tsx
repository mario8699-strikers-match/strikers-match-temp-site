...
import type { PublicPromoterCard } from '@/lib/seoData';
...
interface PromotersPageClientProps {
  initialPromoters: PublicPromoterCard[];
}

export function PromotersPageClient({ initialPromoters }: PromotersPageClientProps) {
  const { t } = useTranslation('promoters');

  const [promoters] = useState<PublicPromoterCard[]>(initialPromoters);
